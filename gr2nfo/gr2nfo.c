...
int main(int argc, char** argv)
{
    ...
    printf("Header info:\n\tMagic:%lu %lu %lu %lu", gr2.header.magic[0], gr2.header.magic[1], gr2.header.magic[2],
           gr2.header.magic[3]);
    ...
    printf(
        ...
        gr2.fileInfo.format, gr2.fileInfo.fileInfoSize, gr2.fileInfo.sectorCount, gr2.fileInfo.totalSize,
        gr2.fileInfo.tag, gr2.fileInfo.crc32, gr2.fileInfo.root.sector, gr2.fileInfo.root.position,
        gr2.fileInfo.type.sector, gr2.fileInfo.type.position);
    ...
    for (size_t i = 0; i < gr2.fileInfo.sectorCount; i++) {
        ...
        printf(
            ...
            sector.alignment, sector.compressedLen, sector.compressType, sector.dataOffset, sector.decompressLen,
            sector.oodleStop0, sector.oodleStop1, sector.fixupOffset, sector.fixupSize, sector.marshallOffset,
            sector.marshallSize);
    }
    ...
    for (size_t i = 0; i < gr2.elements.count; i++) {
        ...
        printf("Element %zu: %u %u %I64u %u %s\n", i, info->rawInfo.arraySize, info->rawInfo.type, info->rawInfo.extra4,
               info->size, info->name);
    }
    ...
    for (size_t i = 0; i < gr2.root->children.count; i++) {
        ...
        printf("Root children %zu: %u %u %I64u %u %s\n", i, info->rawInfo.arraySize, info->rawInfo.type,
               info->rawInfo.extra4, info->size, info->name);
    }
