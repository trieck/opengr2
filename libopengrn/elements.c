...
{
    return type == TYPEID_REFERENCETOARRAY || type == TYPEID_INLINE || type == TYPEID_ARRAYOFREFERENCES || type ==
...
{
    if (type == TYPEID_REFERENCE || type == TYPEID_REFERENCETOARRAY || type == TYPEID_ARRAYOFREFERENCES || type ==
        TYPEID_VARIANTREFERENCE
        || type == TYPEID_REFERENCETOVARIANTARRAY || type == TYPEID_EMPTYREFERENCE || type == TYPEID_STRING)
        return size == 0;
    // string, references and gr2 arrays must have size=0 (as the TypeNode info size is the one of the primitive type)
...
bool Element_New(uint32_t type, const char* name, TElementGeneric** out)
{
    ...
    elem = Element_CreateFromTypeInfo(NULL, &info);
    // we can safetly pass NULL here because nameOffset won't be setted up
