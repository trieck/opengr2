...
static bool Element_ParseNode(TDArray* vptr, TElementGeneric* elem, bool is64, TDArray* global, const uint8_t* data,
                              uint64_t* rootOffset)
{
    ...
    if (elem->rawInfo.type == TYPEID_REFERENCE || elem->rawInfo.type == TYPEID_EMPTYREFERENCE || elem->rawInfo.type ==
        TYPEID_VARIANTREFERENCE) {
        TElementReference* ref = (TElementReference*)elem;
        if (!ref->reference)
            return true;

        return Element_Parse(vptr, typeRoot, (const uint8_t*)ref->reference + ref->offset, is64, global, elem,
                             &newRootOffset);
        ...
        for (uint32_t i = 0; i < ref->base.size; i++) {
            if (!Element_Parse(vptr, typeRoot, (const uint8_t*)ref->data + ref->offset, is64, global, elem,
                               &newRootOffset))
                return false;
        }
...
bool Element_Parse(TDArray* vptr, const uint8_t* type, const uint8_t* data, bool is64, TDArray* global,
                   TElementGeneric* parent, uint64_t* rootOffset)
{
    uint64_t offset = 0;
    TNodeTypeInfo elem;
...
#ifdef _DEBUG
        dbg_printf2("parsed %s type %u offset %I64u root %I64u datasize %u values", newElement->name, elem.type, offset,
                    *rootOffset, newElement->size);
